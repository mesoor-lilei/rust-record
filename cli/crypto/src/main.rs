...
fn main() -> Result<(), Box<dyn Error>> {
    let matches = matches();
    if let Some(item) = matches.subcommand_matches("md5") {
        let mut md5 = Md5::new();
        md5.input_str(item.value_of("text").unwrap());
        println!("密文: {}", md5.result_str());
    } else if let Some(item) = matches.subcommand_matches("aes") {
    ...
    } else if let Some(item) = matches.subcommand_matches("unaes") {
        let key = item.value_of("key").unwrap().as_bytes();
